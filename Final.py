...
boss_bullets = []
boss_attack_cooldown = 120 # Cooldown timer in frames
boss_attack_timer = 0
...
def keyboardListener(key, x, y):
    global turnAngle, xbound, ybound, gameOver, sv_cheats, pov, keys_pressed, status_message, player_on_ground, current_weapon, fire_man_active, Epressed
    ...
    # Update keys_pressed dictionary. Movement is handled in updateVariables.
    if not gameOver:
        if key == b'w':
            theta = math.radians(turnAngle-90)
            xmin, xmax = xbound[0], xbound[1]
            ymin, ymax = ybound[0], ybound[1]
            newx = playerPos[0] + 50*math.cos(theta)
            newy = playerPos[1] + 50*math.sin(theta)
            
            if (newx - 100 >= xmin and newx + 100 <= xmax and newy - 100 >= ymin and newy + 100 <= ymax):
                playerPos[0] += 50*math.cos(theta)
                playerPos[1] += 50*math.sin(theta)
...
def enemy(x,y,z,sf, currentHealth, maxHealth):
    ...
    gluSphere(gluNewQuadric(), radius, 10, 10) # parameters are: quadric, radius, slices, stacks
    ...
    # Foreground (green) - REVERSED
    health_ratio = max(0.0, min(1.0, currentHealth / maxHealth))
    glColor3f(0, 1, 0)
    glBegin(GL_QUADS)
    # Start at the right edge and go left
    glVertex3f(80, 0, 0); glVertex3f(80 * (1 - health_ratio), 0, 0); glVertex3f(80 * (1 - health_ratio), 10, 0); glVertex3f(80, 10, 0)
    glEnd()
    glPopMatrix()
...
def render_health_powerup(x, y, z):
    ...
    glRotatef(45, 0, 0, 1) # Added rotation
...
def render_health_bar():
    ...
    glBegin(GL_QUADS)
    glVertex2f(10, 50); glVertex2f(10 + bar_width, 50); glVertex2f(10 + bar_width, 50 + bar_height); glVertex2f(10, 50 + bar_height)
    glEnd()
...
def render_boss_health_bar():
    ...
    glPushMatrix()
    glLoadIdentity()
    bar_width, bar_height, bar_x, bar_y = 600, 25, (screenX - 600) / 2, screenY - 50
    health_ratio = max(0.0, boss_health / boss_max_health)
    glColor3f(0.5, 0, 0)
    glRectf(bar_x, bar_y, bar_x + bar_width, bar_y + bar_height)
    glColor3f(1, 0, 0)
    glRectf(bar_x, bar_y, bar_x + (bar_width * health_ratio), bar_y + bar_height)
    glPopMatrix()
    glMatrixMode(GL_PROJECTION)
    glPopMatrix()
    glMatrixMode(GL_MODELVIEW)
